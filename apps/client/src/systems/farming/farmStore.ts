...
export interface FarmActions {
  ...
  /**
   * Harvests every mature planted tile at once. Returns an array of
   * { crop, quantity } — one entry per tile. Each harvested tile is reset
   * to tilled so it is immediately ready for replanting.
   */
...
export const useFarmStore = create<FarmState & FarmActions>((set, get) => ({
  ...
  harvestAll: () => {
    ...
    for (const [k, t] of Object.entries(tiles)) {
      ...
      nextTiles[k] = { kind: 'tilled', tilledOnDay: day, watered: false };
    }
    if (yields.length > 0) {
      set({ tiles: nextTiles });
    }
