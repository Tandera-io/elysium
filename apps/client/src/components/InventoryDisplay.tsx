...
// ---------------------------------------------------------------------------
// Inline SVG placeholder icons keyed by item id. Add real asset paths here
// once PNGs are available in src/assets/icons/.
// ---------------------------------------------------------------------------
...
function iconFor(item: InventoryItem): string {
  // If the item already carries an explicit path/data-URL, honour it
...
// ---------------------------------------------------------------------------
// InventoryDisplay — 8 slots arranged in 2 rows of 4
// ---------------------------------------------------------------------------

interface InventoryDisplayProps {
  /** Whether the panel is currently visible. Controlled by the parent (App). */
  open: boolean;
  /** Called when the player clicks the close button or presses I again. */
  onClose: () => void;
}
...
export function InventoryDisplay({ open, onClose }: InventoryDisplayProps) {
  ...
  // Ensure we always render exactly PLAYER_INVENTORY_SIZE slots
