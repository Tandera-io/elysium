      ...
      <div className="flex flex-wrap gap-1.5 justify-center pointer-events-auto">
        ...
        {topicKeys.map((key) => (
          <button
            ...
            disabled={pending}
            className={`text-xs px-3 py-1.5 rounded-full border transition-colors disabled:opacity-50 ${openTopic === key ? 'bg-slate-600 border-slate-500 text-slate-100' : 'bg-slate-800/80 hover:bg-slate-700 border-slate-600 text-slate-300'}`}
          >
