...
//   getRepeatVisitLine(npcId, interactionCount) → string
...
const TIER_THRESHOLDS = {
  first_meeting: { minCount: 0, maxCount: 1, minHeart: 0, maxHeart: 1 },
  repeat_early: { minCount: 1, maxCount: 5, minHeart: 0, maxHeart: 3 },
  repeat_regular: { minCount: 5, maxCount: 20, minHeart: 0, maxHeart: 6 },
  friend: { minCount: 20, maxCount: Infinity, minHeart: 6, maxHeart: Infinity },
};
...
export function classifyContext(context = {}) {
  ...
  // Use thresholds to classify tier
  if (count <= TIER_THRESHOLDS.first_meeting.maxCount - 1) return 'first_meeting';
  if (heart >= TIER_THRESHOLDS.friend.minHeart || count >= TIER_THRESHOLDS.friend.minCount)
    return 'friend';
  if (count >= TIER_THRESHOLDS.repeat_regular.minCount) return 'repeat_regular';
  return 'repeat_early';
}
