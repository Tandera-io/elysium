/**
 * Dorinha's branching dialogue tree — plain JS, no framework dependencies.
 *
 * Shape:
 *   start  — key of the entry node
 *   nodes  — map of node key → { text, choices: [{ id, text, next }] }
 *
 * next === null closes the dialogue when the player picks that choice.
 */

/** @type {{ start: string, nodes: Record<string, { text: string, choices: Array<{ id: string, text: string, next: string|null }> }> }} */
