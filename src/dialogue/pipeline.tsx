/**
 * pipeline.tsx — Dorinha dialogue pipeline component.
 *
 * Wires together:
 *   - Player proximity detection via useDorinhaInteraction
 *   - E key → open choice-tree dialogue
 *   - Renders DorinhaInteractHint when in range (but no dialogue open)
 *   - Renders NPCDialogueOverlay when dialogue is active
 *
 * Usage: mount <DialoguePipeline> once in the app root alongside the scene.
 *
 * Props let callers supply live positions from their own stores so this
 * component stays decoupled from specific store implementations.
 */
import { useEffect } from 'react';
import { DORINHA_ID, DORINHA_NAME, useDorinhaInteraction } from '../npc/Dorinha';
import type { Vec2 } from '../npc/Dorinha';

// ── Minimal types (mirrors apps/client/src/stores/dialogueStore) ──────────────
...
export interface DialoguePipelineProps {
  /** Live player world position (updated each frame). */
  playerPos: Vec2;
  /** Dorinha's world position (from NPC store). */
  dorinhaPos: Vec2;
  /** Dorinha's dialogue tree data. */
  dorinhaTree: DialogueTree;
  /** Injected choice-dialogue store API. */
  store: ChoiceDialogueAPI;
}

/**
 * DialoguePipeline — mounts once in the app root.
 * Handles proximity detection, E key interaction, and dialogue rendering
 * for Dorinha. Inject positions and the store from outside to keep this
 * component testable and store-agnostic.
 */
